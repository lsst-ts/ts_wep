        ...
        for srcOn, idxList in list(enumerate(radIdx)):
            ...
            if len(idxList) == 1:
                ...
                # overlapping sources by minMagDiff then keep it
                    ...
                    index.append(groupIndices[srcOn])
                    sourcesKept += 1
                # If the number of overlapping sources is less than or equal to the
                # max number of blended sources allowed then keep this source
                elif len(magDiff) <= maxBlended:
                    index.append(groupIndices[srcOn])
                    sourcesKept += 1
                # Keep the source if it overlaps with more than maxBlended sources
                # but is at least minMagDiff brighter than all sources dimmer than
                # the maxBlended-th source.
