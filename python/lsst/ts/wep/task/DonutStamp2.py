...
from lsst.geom import Point2D, SpherePoint, Point2I
...
@dataclass
class DonutStamp2(AbstractStamp):
    """Single donut stamp
    ...
    sky_position: SpherePoint
    centroid_position: Point2I
    detector_name: str
    defocalType: DefocalType
    field: Point2D
    pMask: np.array
    cMask: np.array
    offAxisCoeff: np.array
    ...
    def makeMask(self, inst, model, boundaryT, maskScalingFactorLocal):
        ...
        self.cMask = np.ones(dimOfDonut, dtype=int)
    ...
    def makeMaskList(self, inst, model):
        ...
        if model in ("paraxial", "onAxis"):
            ...
            cax, cay, cbx, cby = self._rotateMaskParam(
                maskCa, maskCb, self.field.x, self.field.y
            )
            masklist = np.array(
                [
                    [0, 0, 1, 1],
